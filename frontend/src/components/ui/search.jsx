...
class UsersSearch extends Component {
  ...
  handleChange(e) {
    ...
    users = users.filter(
      (user) => !this.props.selectedUsers.map((user) => user.username).includes(user.username)
    );
    if (users.length) {
    ...
    let results = (
      <ul className={`search-list ${dm}`}>
        {limitedList.map((user) => (
          <li onClick={(e) => this.props.handleUsernameClick(user.username, user.id)} key={user.id}>
            {user.username}
          </li>
        ))}
      </ul>
          ...
          {this.props.selectedUsers.length ? (
            <ul>
              {this.props.selectedUsers.map((user, idx) => (
                <li onClick={(e) => this.props.removeUser(e)} key={idx}>
                  {user.username}
                </li>
              ))}
            </ul>
...
const mapStateToProps = (state) => {
  let users =
    Object.values(state.entities.users).filter((el) => el.id !== state.session.currentUserId) || [];
  return {
    users: users,
  };
};
